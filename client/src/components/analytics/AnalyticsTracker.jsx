import {
  ...
  const MIN_HEARTBEAT_ACTIVE_MS =
    1000;
  
  const MIN_HEARTBEAT_SEND_GAP_MS =
    15 * 1000;
  
  const PAGE_VIEW_CLIENT_DEDUPE_MS =
    1500;
  ...
  function queuePageView(path) {
    ...
    if (
      lastQueuedPageView.signature ===
        signature &&
      now -
        lastQueuedPageView.queuedAt <
